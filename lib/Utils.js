...
exports.subirArchivo2 = function(files, conservarNombre, callback) {
    console.log("******subirArchivo*************");
    console.log("******subirArchivo*************");
    console.log("******subirArchivo*************");
    
    var rutaTmp = files.path;
    var ext = G.path.extname(rutaTmp);
    var nombreArchivo = (conservarNombre) ? files.name : G.random.randomKey(3, 3) + ext;
    var path = G.dirname + ((files.customPath) ? files.customPath :  G.settings.carpeta_temporal);
    var rutaNueva = path + nombreArchivo;
