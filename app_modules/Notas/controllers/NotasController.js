...
Notas.prototype.generarSincronizacionDianDebito = (req, res) => {
    ...
    var DireccionAdquiriente = that.m_direccion_adquiriente;
...
const validarCamposVacios = nota => {
    ...
    if (existe) {
        contiene = nota.length ? nota.length : Object.keys(nota).length;
    }

    if (existe && esIterable && contiene) {
        for (const item in nota) {
            if (validarCamposVacios(nota[item])) {
                delete nota[item];
            }
        }
